...
#            y_fake = y_fake.type(torch.LongTensor)
            ...
            # Clamp values to valid range for BCE loss
            dec1_out = torch.clamp(dec1(fake_sample[:, :-1]).float(), 1e-7, 1 - 1e-7)
            dec2_out = torch.clamp(dec2(fake_wo_sen[:, :-1]).float(), 1e-7, 1 - 1e-7)
            y_fake_clamped = torch.clamp(y_fake.reshape(-1,1).float(), 0, 1)

            loss_d1 = loss_dec(dec1_out, y_fake_clamped)

            loss_d2 = loss_dec(dec2_out, y_fake_clamped)
...
#                y_fake = y_fake > 0.5
                ...
                # Clamp values to valid range for BCE loss
                dec1_out_g = torch.clamp(dec1(fake_sample[:, :-1]).reshape(-1).float(), 1e-7, 1 - 1e-7)
                dec2_out_g = torch.clamp(dec2(fake_wo_sen[:, :-1]).reshape(-1).float(), 1e-7, 1 - 1e-7)
                y_fake_g = torch.clamp(y_fake.float(), 0, 1)

                loss_d1 = loss_dec(dec1_out_g, y_fake_g)
                loss_d2 = loss_dec(dec2_out_g, y_fake_g)
                ...
                if torch.isnan(s1[0]).any():
                    print("Warning: NaN detected in generator parameters")
