from ifgan_GC import train_ifgan
import numpy as np
import data_loading
import utils
import argparse
...
parser.add_argument("--lam", type=float, default=0.1, help="regularisation coef")
...
#import pdb
#pdb.set_trace()
X_in = data_loading.Data_Loading_GC(1)
...
# Based on gender
test_X_female = testX[np.where(testX[:, 5] > 0)]
testY_female = testY[np.where(testX[:, 5] > 0)]
test_X_male = testX[np.where(testX[:, 6] > 0)]
testY_male = testY[np.where(testX[:, 6] > 0)]
