...
def FTU_prob(mlp, X_test, dataset="adult"):
    """Calculate fairness metric FTU"""
    ...
    pred0 = mlp.predict_proba(X_test_0)[:,1]
    pred1 = mlp.predict_proba(X_test_1)[:,1]
    pred0 = pred0.astype(float)
    pred1 = pred1.astype(float)


    ftu = abs(np.mean(pred0) - np.mean(pred1))


    ftuA = np.mean(abs(pred0-pred1))

    return ftu,ftuA


def FTU_alt(mlp, X_test, dataset="adult"):
    """Calculate fairness metric FTU"""
    ...
    if 'ethnicity' in X_test_df:
        ...
        X_test_0 = X_test_df.assign(sex=0)
        X_test_1 = X_test_df.assign(sex=1)
    pred0 = mlp.predict(X_test_0)
    pred1 = mlp.predict(X_test_1)
    pred0 = pred0.astype(float)
    pred1 = pred1.astype(float)

    ftu = np.mean(abs(pred0-pred1))
