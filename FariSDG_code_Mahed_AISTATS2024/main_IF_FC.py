from ifgan_FC import train_ifgan
import numpy as np
import data_loading
import utils
import argparse
...
parser.add_argument("--lam", type=float, default=0.001, help="regularisation coef")
...
#import pdb
#pdb.set_trace()
X_in = data_loading.Data_Loading_FC(1)
#trainX, trainY, testX, testY = data_loading.Data_Loading_FC()

#trainX, trainY, testX, testY = data_loading.Data_Loading_GC()
#print(type(trainX))

X_gen = train_ifgan(X_in, opt)
x_s = X_gen[:, :-1]
y_s = X_gen[:, -1] > 0.5
...
# Based on gender
test_X_female = testX[np.where(testX[:, 8] > 0)]
testY_female = testY[np.where(testX[:, 8] > 0)]
test_X_male = testX[np.where(testX[:, 9] > 0)]
testY_male = testY[np.where(testX[:, 9] > 0)]
...
test_X_male2 = copy.deepcopy(test_X_female)
test_X_male2[:, 8] = 0
test_X_male2[:, 9] = 1
...
test_X_male_com = np.concatenate((test_X_male, test_X_male2), axis=0)
